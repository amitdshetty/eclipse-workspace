...
public class Wikipedia {
	/**
	 * Crawls both English and German Wikipedia. Finds 10 random German Wikipedias
	 * by following redirects through the "special:random" page, and returns the
	 * country of origin for the first IP address (if any) that it finds on the
	 * page. After this, does the same for English Wikipedia edits.
	 * 
	 * @param args
	 */
	// Crawls both English and German Wikipedias. Returns physical address, and
	// datetime edits were made
	...
	private static String getEditCountry(String histPageUrl) {
		...
		for (Element row : rows) {
			System.out.println(row.select(".mwuserlink").text());
