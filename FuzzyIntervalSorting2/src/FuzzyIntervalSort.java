...
public class FuzzyIntervalSort<E extends Comparable<E>> {
	// swap CInterval object at the index, one, with the object at the index,
	// another
	...
	private CInterval<E> findIntersectionWithRandomPivot(ArrayList<CInterval<E>> intervals, int start, int end) {
		...
		for (int i = start; i < end; i++) {
			...
			if ((cur.getStart().compareTo(intersection.getEnd()) <= 0) && (cur.getEnd().compareTo(intersection.getStart())) >= 0) {
				// TODO Change the start position of the intersection if in need
				if (cur.getStart().compareTo(intersection.getStart()) > 0)
					intersection.setStart(cur.getStart());// TODO Change the start position of the intersection if in need
	...
	private int partitionRight(ArrayList<CInterval<E>> intervals, CInterval<E> intersection, int start, int end) {
		int s = start - 1;
		if (intervals == null || intersection == null || start > end || end < 0)
			return -1;
	...
	private int partitionLeftMiddle(ArrayList<CInterval<E>> intervals, CInterval<E> intersection, int upbound, int start, int end) {
		int s = start - 1;
		if (intervals == null || intersection == null || upbound == -1 || start > end || end < 0)
			return -1;
	...
	public void fuzzySort(ArrayList<CInterval<E>> intervals, int start, int end) {
		if (start < end) {
			// TODO Retrieve intersection, named as the CInterval<E> variable intersection
			// based on randomly chosen pivot
			...
			// TODO Recursively fuzzy sorting the left part
			// which locate absolutely after the intersection
	...
	public static void main(String[] argv) {
		...
		while (keyboard.hasNextLine()) {
			...
			if (line.indexOf(",") >= 0) {
				...
				list.add(new CInterval<Integer>(new Integer(intervalStr[0].trim()), new Integer(intervalStr[1].trim())));
			} else
				break;
		}
